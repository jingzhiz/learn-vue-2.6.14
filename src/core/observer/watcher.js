 ...
 */
/*
  # 一个组件一个 watcher（渲染 watcher）或者一个 watcher（用户 watcher）
...
export default class Watcher {
  ...
  */
  ...
  update() {
    ...
    if (this.lazy) {
      /*
        # 将 dirty 置为 true，可以让 computedGetter
        # 执行时重新计算 computed 回调函数的执行结果
      */
      this.dirty = true;
    } else if (this.sync) {
      /*
      ...
      */
      ...
      // # 更新时一般走这里，将 watcher 放入 watcher 队列
      queueWatcher(this);
    }
  }
  ...
  run() {
    if (this.active) {
      ...
      if (
        ...
        // have mutated.
        // # 即使对象/数组的值相同，深度观察者和对象上的观察者也应该启动，因为值可能发生了变化。
        isObject(value) ||
        this.deep
      ) {
        // set new value
