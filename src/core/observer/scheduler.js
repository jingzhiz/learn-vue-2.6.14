...
/*
  ...
  #   1、更新 flushing 为 true，表示正在刷新队列
  #     在此期间往队列中 push 新的 watcher 时需要特殊处理（将其放在队列的合适位置）
...
function flushSchedulerQueue() {
  ...
  */
  ...
  /*
    # 这里直接使用了 queue.length，动态计算队列的长度，没有缓存长度，
  ...
  */
  for (index = 0; index < queue.length; index++) {
    ...
    /*
      ...
      # 或者获取 this.xx（xx 为用户 watch 的第二个参数），当然第二个参数也有可能是一个函数，那就直接执行
    */
    watcher.run();
  ...
  /*
   ...
   #     waiting = flushing = false，表示可以像 callbacks 数组中放入新的 flushSchedulerQueue 函数，
  ...
  if (has[id] == null) {
    ...
    if (!flushing) {
      ...
      /*
        # 当前在刷新队列
        # 从队列末尾开始倒序遍历
        #   根据当前 watcher.id 找到它大于的 watcher.id 的位置
        #   然后将自己插入到该位置之后的下一个位置
